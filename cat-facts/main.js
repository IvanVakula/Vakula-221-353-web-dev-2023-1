...
function setAutocomplete(event) {
    ...
    doSearch();
    let autocompleteContainer = document.querySelector('.auto-list');
...
function showAutocomplete (event) {
    let autolist = document.querySelector('.auto-list');
    let url = new URL(autolist.dataset.url);
    let findqueryauto = document.querySelector('.search-field').value;
    url.searchParams.append('q', findqueryauto);
    let xhr = new XMLHttpRequest();
    xhr.open('GET', url);
    xhr.responseType = 'json';
    xhr.onload = function () {
